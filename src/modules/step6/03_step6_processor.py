...
class Step6Processor:
    ...
    async def process_single_group_gemini(self, group_key: str, group_data: Dict, 
        ...
        Returns:
            Dict: Gemini OCR処理結果
        """
        logger.info(f"Step6-01: OCR処理 ({group_index}/{total_groups}) {group_key}")
        ...
        try:
            ...
            if not ocr_result.get("success"):
                logger.warning(f"OCR処理失敗: {group_key} - {ocr_result.get('error', 'unknown error')}")
                return {
                    "group_key": group_key,
                    "success": False,
            ...
            # テキスト結果を保存
            ocr_output_dir = session_dirs.get("ocr_results", session_dirs["final_results"])
            ...
            if save_result["success"]:
                ...
                logger.warning(f"Step6-01: テキスト保存で一部エラー ({group_key}): {save_result['errors']}")
    ...
    async def process_single_group_document_ai(self, group_key: str, group_data: Dict, 
        ...
        """
        logger.info(f"Step6-04: Document AI OCR処理 ({group_index}/{total_groups}) {group_key}")
        ...
        try:
            ...
            if not doc_ai_result.get("success"):
                logger.warning(f"Document AI OCR処理失敗: {group_key} - {doc_ai_result.get('error', 'unknown error')}")
                return {
                    "group_key": group_key,
                    "success": False,
            ...
            # Document AI結果を保存
            doc_ai_output_dir = session_dirs.get("document_ai_results", session_dirs["final_results"])
            ...
            if save_result["success"]:
                logger.info(f"Step6-04: 完了!! ({group_key}: Document AIテキスト保存完了)")
            else:
                logger.warning(f"Step6-04: Document AIテキスト保存で一部エラー ({group_key}): {save_result['errors']}")
    ...
    async def process_ocr_groups(self, ocr_groups: Dict, session_dirs: Dict) -> Dict:
        ...
        logger.info(f"Step6処理開始: {len(groups)}グループ対象 (Gemini + Document AI並行処理、最大{self.max_concurrent_ocr}並列)")
        
        # Document AIディレクトリの作成
        if "document_ai_results" not in session_dirs:
            # session_dirsから既存のベースディレクトリを使用
            base_dir = session_dirs.get("final_results") or session_dirs.get("converted_images", "")
            if base_dir:
                parent_dir = os.path.dirname(base_dir)
                session_dirs["document_ai_results"] = os.path.join(parent_dir, "document_ai_results")
            else:
                logger.warning("Document AIディレクトリの作成に失敗しました。final_resultsディレクトリを使用します")
                session_dirs["document_ai_results"] = session_dirs.get("final_results", "")
        ...
        async def process_group_with_both_engines(item):
            ...
            async with semaphore:
                ...
                gemini_task = self.process_single_group_gemini(
                    group_key, group_data, session_dirs, group_index, len(groups)
                )
        ...
        # 全グループを並列処理
        tasks = [process_group_with_both_engines(item) for item in groups.items()]
        ...
        for i, combined_result in enumerate(combined_results):
            if isinstance(combined_result, Exception):
                ...
                gemini_failed_results.append({"group_key": group_key, "error": str(combined_result)})
                document_ai_failed_results.append({"group_key": group_key, "error": str(combined_result)})
                continue
            ...
            if isinstance(gemini_result, Exception):
                logger.error(f"グループ{group_key}: Gemini処理でエラー - {gemini_result}")
                gemini_failed_results.append({"group_key": group_key, "error": str(gemini_result)})
                ...
                gemini_failed_results.append(gemini_result or {"group_key": group_key, "error": "Gemini処理失敗"})
            
            # Document AI結果の処理
            if isinstance(document_ai_result, Exception):
                logger.error(f"グループ{group_key}: Document AI処理でエラー - {document_ai_result}")
                document_ai_failed_results.append({"group_key": group_key, "error": str(document_ai_result)})
            elif document_ai_result and document_ai_result.get("success"):
                document_ai_successful_results.append(document_ai_result)
                total_document_ai_files.extend(document_ai_result.get("saved_files", []))
            else:
                document_ai_failed_results.append(document_ai_result or {"group_key": group_key, "error": "Document AI処理失敗"})
        ...
        gemini_processing_summary = self.text_manager.create_group_summary(gemini_ocr_summary_data)
        ...
        document_ai_processing_summary = self.document_ai_manager.create_processing_summary(document_ai_summary_data)
        ...
        logger.info(f"--- Step6: 完了!! Gemini={gemini_successful_groups}グループ・{total_gemini_text_files}ファイル, Document AI={document_ai_successful_groups}グループ・{total_document_ai_text_files}ファイル ---")
    ...
    async def process_step5_results(self, step5_results: Dict, session_dirs: Dict) -> Dict:
        ...
        if "ocr_results" not in session_dirs:
            ...
            if base_dir:
                parent_dir = os.path.dirname(base_dir)
                session_dirs["ocr_results"] = os.path.join(parent_dir, "ocr_results")
            else:
                logger.warning("OCRディレクトリの作成に失敗しました。final_resultsディレクトリを使用します")
                session_dirs["ocr_results"] = session_dirs.get("final_results", "")
    ...
    def get_text_extraction_status(self, step6_results: Dict) -> Dict:
        ...
        processing_summary = step6_results.get("step6_results", {}).get("processing_summary", {})
        ...
        return {
            ...
            "success_rate": (
                ...
                max(1, statistics.get("total_groups_processed", 0) + statistics.get("total_groups_failed", 0))
            ) * 100
        }