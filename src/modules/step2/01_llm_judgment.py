...
class LLMJudgment:
    ...
    def __init__(self, config: Dict):
        ...
        Args:
            config (Dict): LLM設定
        """
        self.config = config.get('llm_evaluation', {}).get('judgment', {})
        ...
        logger.debug(f"LLMJudgment初期化: {self.provider}/{self.model}")
    ...
    def _parse_llm_response(self, response_text: str) -> Dict:
        ...
        try:
            ...
            # 必要なキーの存在確認
            required_keys = ['has_something_out_of_document', 'needs_dewarping', 'readability_issues']
    ...
    async def evaluate_dewarping_need(self, image_path: str, prompts: Dict) -> Dict:
        """
        画像の歪み補正必要性を判定
        ...
        Returns:
            Dict: 判定結果
        """
        logger.debug(f"LLM歪み判定開始: {os.path.basename(image_path)}")
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"LLM判定失敗: {last_error} (最大{self.max_retries}回試行)"
            }
    ...
    def save_result(self, result: Dict, output_file: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"判定結果保存: {os.path.basename(output_file)}")
            return True
    ...
    def get_judgment_summary(self, result: Dict) -> Dict:
        ...
        return {
            ...
            "has_out_of_document": judgment.get("has_something_out_of_document", False),
            "readability_level": judgment.get("readability_issues", "unknown"),
            "confidence": judgment.get("confidence_score", 0.0),
            "requires_reprocessing": judgment.get("readability_issues", "").lower() == "major"
        }